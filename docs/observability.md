...
Caught feature failures that produce guidance instead of a result use `record_handled_failure(error)` inside the owning operation. A successful error reply does not make the feature successful; a later escaping exception supersedes the handled failure. HTTP boundaries classify the actual response: expected client refusals are rejected, dependency outages unavailable, timeouts distinct, and unhandled server defects unexpected. A size refusal uses a narrow exception type; general media or validation exceptions remain diagnostic failures.

Optional ecosystem maintenance has named `ecosystem.events`, `ecosystem.refresh` and `telegram.auto_unpin` operations. Network failures use fixed DNS, TLS, connection and HTTP categories; Telegram recovery and telemetry share fixed error descriptions. Never add raw exception messages to explain these categories. The Trouble desk separates actionable formatting/media rejections from ordinary policy refusals, and counts terminal logical Telegram calls after retry policy, not individual wire attempts.
