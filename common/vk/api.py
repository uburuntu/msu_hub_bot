...
class VkApi(VkApiCaller):
    ...
    async def _offset_requests(self, method, max_count: int, count: int = None) -> dict:
        ...
        coros = (
            method(offset=offset, count=min(count - offset, max_count))
            for offset in range(max_count, count, max_count)
        )
    ...
    async def get_groups(self, group_ids: List[Union[int, str]], fields: str = '') -> List[dict]:
        _fields_full: Final = (
            'city', 'country', 'place', 'description', 'wiki_page', 'market', 'members_count', 'counters', 'start_date', 'finish_date',
            'can_post', 'can_see_all_posts', 'activity', 'status', 'contacts', 'links', 'fixed_post', 'verified', 'site', 'ban_info', 'cover',
        )
        max_count: Final = 500
    ...
    async def get_newsfeed(self, owner_ids: Iterable[int], from_ts: int = 0) -> Tuple[dict, dict]:
        ...
        method = partial(self.request, 'newsfeed.get',
                         filters='post', return_banned=1, start_time=from_ts, source_ids=source_ids, count=count)
