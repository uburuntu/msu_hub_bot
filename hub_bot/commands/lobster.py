...
from common.utils import image_bytes_io, megabytes, FakeBytesIO
from hub_bot.utils.caption_layout import CaptionLayoutError, demotivator_caption, demotivator_image, frame_border, lobster_image, MAX_IMAGE_SIDE
...
def demotivator_video(file: io.BytesIO, width: int, text: str) -> io.BytesIO:
    ...
    with TemporaryDirectory(prefix='hub-caption-') as directory:
        ...
        parameters = [
            '-i', str(path), '-filter_complex', filters, '-map', '[out]', '-map', '0:a?',
            '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-movflags', '+faststart', '-f', 'mp4',
        ]
