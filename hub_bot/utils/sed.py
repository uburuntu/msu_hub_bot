...
def _calculate(text, commands):
    ...
    for substitution in substitutions:
        ...
        try:
            text = re.sub(
                substitution['pattern'].replace(r'\/', '/'),
                substitution['sub'].replace(r'\/', '/'),
                text, flags=flags,
            )[:MAX_TEXT_LENGTH]
...
def sed_calc(text, commands, limit=MAX_COMMANDS):
    ...
    try:
        result = subprocess.run(
            ...
            check=True, timeout=SED_TIMEOUT,
        )
