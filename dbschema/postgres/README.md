...
Migration 008 adds a bounded job-overview RPC for registered feature/kind pairs. It returns only queue counts and oldest-due timestamps within the authenticated principal's bot/application scope; payloads, job keys and leases remain private. It does not change retention or the data API contract version.
...
| `vk/subscriptions` | `owner_id:chat_id`, with `:topic:thread_id` for a topic target. The parent index identifies the destination chat/topic. Versioned models preserve omitted options and cursors; administrative edits use guarded writes. Suspension and archival retain the record indefinitely. Automatic publication is disabled; configuration never activates it. |
