...
class AntiBot(CallbackCommandBase):
    ...
    def keyboard(cls, chat_id: int, user_id: int) -> InlineKeyboardMarkup:
        ...
        keyboard.row(
            ...
            InlineKeyboardButton(
                text="🆗 Оставить", callback_data=AntiBotCallback(action="ignore", chat_id=chat_id, user_id=user_id).pack()
            ),
        )
    ...
    async def process_cb(cls, query: CallbackQuery, callback_data: AntiBotCallback, bot: Bot, redis: RedisStorage) -> Message | bool | None:
        ...
        if (
            ...
            or query.message.chat.id != chat_id
            or user_id <= 0
            or action not in {"ban", "kick", "ignore", "close"}
        ):
            return await query.answer("Эта кнопка больше не работает.")
    ...
    async def _process_cb_locked(
        ...
        if not isinstance(bot_member, (ChatMemberOwner, ChatMemberAdministrator)):
            return await query.answer(
                "❇️ Боту нужны права на бан и удаление сообщений в этом чате", cache_time=cls.cache_time_10s, show_alert=True
            )
        ...
        if target.status not in (ChatMemberStatus.MEMBER, ChatMemberStatus.RESTRICTED) or (
            isinstance(target, ChatMemberRestricted) and not target.is_member
        ):
