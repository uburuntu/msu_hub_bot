...
from msu_hub_bot.providers import dvach as api2ch
...
from msu_hub_bot.utils import PriorityQueue, megabytes, one_liner
...
class Dvach(CallbackCommandBase):
    ...
    def keyboard(cls, board: str, urls: Iterable[str]) -> InlineKeyboardMarkup:
        keyboard = (
            ...
            .row(InlineKeyboardButton(text="Обновить 🔄", callback_data=DvachCallback(board=board, url="update").pack()))
            .row()
        )
    ...
    async def process_cb(
        cls, query: CallbackQuery, callback_data: DvachCallback, dvach: api2ch.Api2chAsync, bot: BotWrapper
    ) -> Message | bool | None:
        m = query.message
        ...
        return await bot.send_super_message(
            full_text, web_preview, photos_urls, video_urls, m.chat.id, m.message_id, message_thread_id=m.message_thread_id
        )
