...
def format_text(value: str | Text | None, entities: Sequence[MessageEntity] | None, max_bytes: int) -> FormattedText:
    if isinstance(value, Text):
        if entities is not None:
            raise ResponseError("Use either Text or entities, not both.")
        ...
        while pending:
            ...
            if count > MAX_ENTITIES or depth > 32:
                raise ResponseLimitError("Formatting exceeds the nesting or work budget.")
            for child in node:
                count += 1
                ...
                if isinstance(child, Text):
                    ...
                    try:
                        ...
                        raise ResponseError("The response contains invalid Unicode.") from None
                    if length > max_bytes:
                        raise ResponseLimitError("The response exceeds the configured byte budget.")
        try:
            text, rendered = value.render()
        except ValueError:
            raise ResponseError("Invalid response formatting.") from None
        entities = rendered
    elif value is None or isinstance(value, str):
        text = value or ""
    else:
        raise ResponseError("Response text must be a string or aiogram Text.")
    ...
    for entity in copied:
        ...
        if entity.type not in MessageEntityType or entity.offset < 0 or entity.length <= 0 or end > size:
            raise ResponseError("Invalid response entity boundaries.")
        if (
            ...
            raise ResponseError("Response entity metadata is missing.")
        while stack and entity.offset >= stack[-1]:
            stack.pop()
        if stack and end > stack[-1]:
            raise ResponseError("Response entities cross rather than nest.")
        stack.append(end)
        if len(stack) > 32:
            raise ResponseLimitError("Formatting exceeds the nesting or work budget.")
    ...
    if wanted:
        for char in text:
            ...
            if not wanted:
                break
    if wanted:
        raise ResponseError("An entity boundary splits a Unicode character.")
...
def split_text(
    ...
    while start < len(text):
        ...
        if boundary <= start_units:
            raise TextNeedsFile("An indivisible entity exceeds the message limit.")
        ...
        part = text[start:end]
        if not part.strip():
            raise TextNeedsFile("Whitespace-only chunks require file delivery.")
        adjusted = tuple(
            entity.model_copy(
                update={
